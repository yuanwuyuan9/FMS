...
class FMS(nn.Module):
    def __init__(self, args, n_relations, params_for_neighbors, n_entity):
        ...
        self.MLP = MLP(dim=self.hidden_dim, time_varying=True, w=self.cfm_model_dim, out_dim=self.hidden_dim).to(device)
        self.Linear_out_head = nn.Linear(self.hidden_dim * 3, n_entity)
    ...
    def forward(self, batch):
        ...
        self.relations = batch['relations']
    ...
    def _call_model(self):
        ...
        if self.use_context:
            ...
            edge_list_head, mask_list_head = self._get_neighbors_and_masks(self.relations, self.entity_pairs[:, 0],
                                                                           self.train_edges)

            edge_list_tail, mask_list_tail = self._get_neighbors_and_masks(self.relations, self.entity_pairs[:, 1],
                                                                           self.train_edges)
            ...
            self.relation_emb = self.relation_features[self.relations]

            x0 = self.aggregated_neighbors_head
            x1 = self.aggregated_neighbors_tail

            t, xt, ut, y0, y1, _ = self.cfm.guided_sample_location_and_conditional_flow(x0, x1, x0, x1, return_noise=True)
            ...
            output += self.Linear_out_head(torch.cat((self.aggregated_neighbors_head, self.aggregated_neighbors_tail, vt_pred), dim=-1))
    ...
    def _get_neighbors_and_masks(self, relations, entity_pairs, train_edges):
        ...
        masks = []
        # Entity predictions do not need to mask associated edges
        ...
        for i in range(self.context_hops):
            ...
            mask = torch.ones_like(neighbor_edges).float()
            masks.append(mask)
    ...
    def _aggregate_neighbors(self, edge_list, mask_list):
        ...
        res = edge_vectors[0].view([self.batch_size, self.hidden_dim])
    ...
    def train_step(model, optimizer, batch):
        ...
        pos_scores = model.scores.cuda()[
            [torch.arange(len(model.scores), device='cuda'), model.labels.to(device='cuda', dtype=torch.long)]]
        max_n = torch.max(model.scores, 1, keepdim=True)[0]
        loss = torch.sum(- pos_scores + max_n + torch.log(torch.sum(torch.exp(model.scores - max_n), 1))) + model.lamda * model.loss_cfm
        loss.backward()
        # loss = torch.mean(criterion(model.scores, model.label))
        # loss.backward()
        optimizer.step()
    ...
    def test_step(model, batch):
        ...
        with torch.no_grad():
            ...
            return correct_predictions.cpu(), model.scores
